...
HelmUtils.installHelmRelease()
{
    ...
    values=$3
    namespace=$4
    helm install "${SOURCE_FOLDER}/${chartPath}" --tiller-namespace ${namespace} --set fullnameOverride=${releaseName},PROJECT_ID=${PROJECT_ID},VERSION='latest'   pm --name ${releaseName}
    if [ $? -eq 0 ]; then
